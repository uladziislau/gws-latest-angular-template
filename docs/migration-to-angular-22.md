...
* **`@angular/*` (включая `cdk`, `material`, `router` и др.) — Версия `^22.0.0`**
...
* **`@types/node` — Версия `^22.10.0` (или `^22.22.2`)**
  * *Обоснование:* Описание типов Node.js API для TypeScript. Версия подобрана
    под ветку рантайма Node 22. Позволяет компилировать скрипты или запускать
    тесты (например, с Vitest) без ошибок типизации. Пакет находится в `devDependencies`.
...
- Обнови все пакеты `@angular/*` (включая cdk, material) до стабильной
  версии `^22.0.0`.
...
Поскольку в Google AI Studio **нет терминала** для пользователя, выполнение всех команд (таких как установка зависимостей или запуск сборки) должно производиться самим ИИ-агентом.

Если вы хотите применить патч или запустить команду, просто попросите об этом ИИ-агента в чате:
1. Попросите ИИ-агента выполнить чистую установку (например: *"Выполни `npm ci` и убедись, что CLI пропатчен"*). Агент проверит, появился ли файл `node_modules/@angular/cli/src/utilities/node-version.js` и пропатчен ли он выводом: `Successfully patched Angular CLI node version check`.
2. Попросите ИИ собрать проект для проверки: *"Собери проект, выполнив `npm run build`"*.

*Если сборка завершилась выводом файлов сборщика в директорию `dist/`, проект успешно готов к работе под Angular 22!*
